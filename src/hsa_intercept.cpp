...
// RTL_MODE: "full" (default) = signal injection + GPU timing
...
static void queue_intercept_cb(const void* in_packets, uint64_t count,
    ...
    // Observe mode: record kernel names and counts without modifying packets.
    // No signal injection, no timing — just dispatch counting per kernel.
...
static void shutdown() {
    ...
    if (g_observed.load() > 0) {
        fprintf(stderr, "  observed (no sig):   %" PRIu64 "\n", g_observed.load());
    }
...
extern "C" bool OnLoad(void* pTable,
    ...
    // Check RTL_MODE=observe for lightweight kernel counting
